...
python -m pytest world_model/tests recovery/tests grading/tests harness/tests   # all checks (78)
python -m world_model.run                          # generate the synthetic world (plants the hidden segment)
python -m harness.run --n 50                       # build 50 observation+card instances -> harness/data/run
python -m grading.run --run-dir harness/data/run   # grade recovery vs. ground truth (precision/recall, magnitude bias)
